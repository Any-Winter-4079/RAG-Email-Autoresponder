...
# python eval/plot/plot_oracle_final_answerability.py lm_cleaned_text_chunks --split-name dev --email-rrf-timestamp 2026-05-18_23-51-17 --reranker-timestamp 2026-05-17_13-48-14 --both-rrf-timestamp 2026-05-18_22-00-10
...
def get_retrieval_output_paths(oracle_data):
    ...
    return [
        resolve_path(retrieval_output_path, project_root)
...
def plot_final_answerability(data_variant, split_name, strategy_statistics, output_path=None):
    ...
    fig.suptitle("Oracle final answerability results", fontsize=16, y=0.985)
    fig.text(
        0.5,
        0.91,
    ...
    fig.text(
        0.5,
        0.86,
        f"email-only RRF oracle: {strategy_statistics['email_rrf']['timestamp']}, "
        f"cross-source reranker oracle: {strategy_statistics['reranker']['timestamp']},",
        ha="center",
        fontsize=16,
    )
    fig.text(
        0.5,
        0.81,
        f"cross-source RRF oracle: {strategy_statistics['both_rrf']['timestamp']})",
        ha="center",
        fontsize=16,
    )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Oracle split name. Defaults to the currently configured split.",
    )
    parser.add_argument(
        "--email-rrf-timestamp",
        required=True,
