...
# python eval/plot/plot_oracle_data_variant_answerability.py --raw-chunks 2026-05-17_01-46-08 --manually-cleaned-chunks 2026-05-17_03-39-42 --lm-cleaned-text-chunks 2026-05-17_13-48-14 --lm-summary-chunks 2026-05-17_15-01-22 --lm-q-and-a-chunks 2026-05-17_16-11-49 --lm-q-and-a-for-q-only-chunks 2026-05-17_17-12-09
...
VARIANT_TO_METADATA_DISPLAY = {
    "raw_chunks": "raw",
    "manually_cleaned_chunks": "manually cleaned",
    "lm_cleaned_text_chunks": "LM cleaned",
    "lm_summary_chunks": "LM summary",
    "lm_q_and_a_chunks": "LM Q&A",
    "lm_q_and_a_for_q_only_chunks": "LM Q&A Q-only",
}
VARIANT_TO_ARGUMENT = {
...
def get_retrieval_timestamp(oracle_data):
    metadata = oracle_data.get("oracle_input_metadata") or {}
    retrieval_output_paths = metadata.get("retrieval_output_paths") or []
    retrieval_timestamps = [
        ...
        for timestamp in [Path(str(retrieval_output_path)).parent.parent.name]
    ]
    if not retrieval_timestamps:
        return "unknown"
    return " / ".join(dict.fromkeys(retrieval_timestamps))
...
def collect_variant_statistics(oracle_paths):
    ...
    for variant, oracle_path in oracle_paths.items():
        ...
        variant_statistics[variant] = {
            ...
            "retrieval_timestamp": get_retrieval_timestamp(oracle_data),
            "oracle_timestamp": get_oracle_timestamp(oracle_path),
            "counts": counts,
...
def get_plot_metadata(variant_statistics):
    retrieval_timestamps = [
        variant_statistics[variant]["retrieval_timestamp"]
        for variant in VARIANT_ORDER
    ]
    retrieval_timestamp = " / ".join(dict.fromkeys(retrieval_timestamps))
    first_oracle_timestamps = ", ".join(
        f"{VARIANT_TO_METADATA_DISPLAY[variant]}: {variant_statistics[variant]['oracle_timestamp']}"
        for variant in VARIANT_ORDER[:2]
    )
    second_oracle_timestamps = ", ".join(
        f"{VARIANT_TO_METADATA_DISPLAY[variant]}: {variant_statistics[variant]['oracle_timestamp']}"
        for variant in VARIANT_ORDER[2:4]
    )
    third_oracle_timestamps = ", ".join(
        f"{VARIANT_TO_METADATA_DISPLAY[variant]}: {variant_statistics[variant]['oracle_timestamp']}"
        for variant in VARIANT_ORDER[4:]
    )
    return (
        f"(retrieval: {retrieval_timestamp}\n"
        f"oracle: {first_oracle_timestamps}\n"
        f"oracle: {second_oracle_timestamps}\n"
        f"oracle: {third_oracle_timestamps})"
    )
...
def plot_answerability_score(ax, variant_statistics):
    ...
    ax.set_title("Query-level data variant answerability score")
...
def save_variant_plot(variant_statistics, output_path=None):
    ...
    fig.suptitle(
        ...
        f"{get_plot_metadata(variant_statistics)}",
        fontsize=16,
    )
...
def main():
    ...
    oracle_paths = {
        variant: resolve_oracle_path_from_timestamp(
            project_root,
            DATA_VARIANT_TEST_SPLIT_NAME,
            variant,
            getattr(args, VARIANT_TO_ARGUMENT[variant]),
            "plot_oracle_data_variant_answerability",
        )
        for variant in VARIANT_ORDER
    }
    ...
    for variant in VARIANT_ORDER:
        ...
        summary_lines.append(
            f"\t{variant}: path={format_path(statistics['oracle_path'], project_root)}, "
