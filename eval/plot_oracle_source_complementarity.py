...
# python eval/plot_oracle_source_complementarity.py
# python eval/plot_oracle_source_complementarity.py --web-oracle-path eval/results/run_oracle_discriminator/dev/<timestamp>/<variant>/oracle_discriminator.json --email-oracle-path eval/results/run_oracle_discriminator/dev/<timestamp>/<variant>/oracle_discriminator.json
...
def resolve_existing_path(candidate_path, path_name):
    oracle_path = Path(candidate_path)
    if not oracle_path.is_absolute():
        oracle_path = Path(project_root) / oracle_path
    if not oracle_path.exists():
        raise ValueError(
            f"plot_oracle_source_complementarity: {path_name} file does not exist:\n"
            f"\t{oracle_path}"
        )
    return oracle_path
...
def infer_oracle_source(oracle_data):
    ...
    if metadata.get("collection_name"):
        collection_names.append(str(metadata["collection_name"]))
    collection_names.extend(
        str(collection_name)
        for collection_name in metadata.get("collection_names") or []
    )
    collection_paths = []
    if metadata.get("collection_dump_path"):
        collection_paths.append(str(metadata["collection_dump_path"]))
    collection_paths.extend(
        str(collection_path)
        for collection_path in metadata.get("collection_dump_paths") or []
    )
    metadata_text = " ".join(collection_names + collection_paths)
    if "email_lm_" in metadata_text or "/email_" in metadata_text:
        return "email"
    if "lm_" in metadata_text:
        return "web"

    return None

def resolve_default_oracle_path(source_name):
    oracle_paths = sorted(get_oracle_results_root().rglob("oracle_discriminator.json"))
    if not oracle_paths:
        raise ValueError(
            "plot_oracle_source_complementarity: no oracle files found under:\n"
            f"\t{get_oracle_results_root()}"
        )
    ...
    for oracle_path in oracle_paths:
        ...
        if infer_oracle_source(oracle_data) != source_name:
            continue
        n_samples = oracle_data.get("n_samples") or len(oracle_data.get("results") or [])
        if n_samples > best_n_samples or (
                n_samples == best_n_samples
                and (best_path is None or oracle_path > best_path)):
            best_path = oracle_path
            best_n_samples = n_samples

    if best_path is None:
        raise ValueError(
            "plot_oracle_source_complementarity: no oracle file found for source:\n"
            f"\t{source_name}"
        )

    return best_path

def resolve_oracle_paths(web_candidate_path=None, email_candidate_path=None):
    web_oracle_path = (
        resolve_existing_path(web_candidate_path, "web oracle")
        if web_candidate_path
        else resolve_default_oracle_path("web")
    )
    email_oracle_path = (
        resolve_existing_path(email_candidate_path, "email oracle")
        if email_candidate_path
        else resolve_default_oracle_path("email")
    )
    return web_oracle_path, email_oracle_path
...
def save_comparison_plot(web_oracle_path, email_oracle_path, output_path, comparison_statistics):
    ...
    fig.suptitle(
        "Oracle source-based answerability results\n"
        "(LM summary chunks)",
        fontsize=16,
    )
...
def main():
    ...
    parser.add_argument(
        "oracle_paths",
        nargs="*",
        help="Optional positional paths: web oracle path followed by email oracle path.",
    )
    parser.add_argument(
        "--web-oracle-path",
        default=None,
    ...
    if args.oracle_paths and len(args.oracle_paths) != 2:
        raise ValueError(
            "plot_oracle_source_complementarity: expected either zero positional paths "
    ...
    web_oracle_path, email_oracle_path = resolve_oracle_paths(
        web_candidate_path=web_candidate_path,
        email_candidate_path=email_candidate_path,
    )
    if args.output_path:
    ...
    web_data = load_oracle_data(web_oracle_path)
    email_data = load_oracle_data(email_oracle_path)
    ...
    print(
        ...
        f"\tweb oracle path: {format_path(web_oracle_path)}\n"
        f"\temail oracle path: {format_path(email_oracle_path)}\n"
