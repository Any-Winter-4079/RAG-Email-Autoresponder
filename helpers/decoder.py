...
################################
# Helper 14: Count text tokens #
################################
def count_text_tokens(texts, model_name_or_path):
...
#######################################
# Helper 15: Truncate texts to tokens #
#######################################
def truncate_texts_to_tokens(texts, max_tokens, model_name_or_path):
    tokenizer = get_tokenizer(model_name_or_path)
    return [
        truncate_to_tokens(tokenizer, text, max_tokens)
        for text in texts
    ]

##################################
# Helper 16: Run local LM or VLM #
##################################
