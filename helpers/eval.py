...
def build_base_data_variant_to_source_to_encoder_settings(
        base_data_variant_to_settings=None,
        data_sources=None,
        source_to_encoder_settings=None,
        ):
    if base_data_variant_to_settings is None:
        base_data_variant_to_settings = DATA_VARIANT_TEST_EVAL_VARIANTS
    if data_sources is None:
        data_sources = DATA_VARIANT_EVAL_SOURCES

    base_data_variant_to_source_to_encoder_settings = {}
    for base_data_variant, eval_variant_settings in base_data_variant_to_settings.items():
        ...
        for source_name in data_sources:
            ...
            if source_to_encoder_settings is None:
                encoder_name_to_settings = eval_variant_settings["encoders"]
            else:
                encoder_name_to_settings = source_to_encoder_settings[source_name]
...
def attach_split_samples_to_retrieval_output(retrieval_output, rewritten_split_samples):
    ...
    for split_sample, result in zip(rewritten_split_samples, retrieval_output["results"]):
        result_with_sample = {
            ...
            "anonymized_request": result.get("anonymized_request"),
            "sample": split_sample,
    ...
    def load_single_retrieval_oracle_inputs(retrieval_output_name):
        if retrieval_output_name == "reranker":
            ...
            for result_entry in retrieval_results:
                ...
                anonymized_request = result_entry.get("anonymized_request")
        ...
        for result_index, first_result_entry in enumerate(first_source_results):
            ...
            anonymized_request = first_result_entry.get("anonymized_request")
            chunks = []
            id_to_chunk = {}
