...
def build_finetune_rows(
    ...
    def add_chunks_to_intermediate_rows(
            source_type,
            chunk_field_name,
            target_field_name,
            source_field_name,
            ):
        data_variant_to_source_to_results = (
            data_variant_to_source_to_encoder_results
            if source_type == "m3"
            else data_variant_to_source_to_oracle_results
        )
        for data_variant, source_to_results in data_variant_to_source_to_results.items():
            for data_source, results in source_to_results.items():
                result_groups = results.items() if source_type == "m3" else [(None, results)]
                for encoder_name, result_group in result_groups:
                    source = (
                        f"m3:{data_variant}:{data_source}:{encoder_name}"
                        if source_type == "m3"
                        else f"oracle:{data_variant}:{data_source}"
                    )
                    for result in result_group:
                        if source_type == "m3" and result.get("retrieval_failed"):
                            continue
                        if source_type == "oracle" and result.get("generation_failed"):
                            continue
                        email_sample_key = build_email_sample_key(result["sample"])
                        ...
                        if source_type == "m3":
                            chunk_groups = [result[chunk_field_name][:top_k_retrieval_mined_negatives_per_file]]
                        else:
                            chunk_groups = []
                            subqueries = (
                                result.get("discriminator_result") or {}
                            ).get("subqueries") or []
                            for subquery in subqueries:
                                if target_field_name == "positives" and subquery["answerability"] not in {"0", "1"}:
                                    continue
                                chunk_groups.append(subquery[chunk_field_name])
                        for chunks in chunk_groups:
                            for chunk in chunks:
                                payload = (
                                    chunk.get("retrieval_result") or chunk
                                ).get("payload")
                                if not payload:
                                    continue
                                text = get_text_from_payload(payload)
                                if target_field_name == "negatives" and (not text or text in intermediate_row["positives"]):
                                    continue
                                intermediate_row[target_field_name].add(text)
                                add_text_by_source(intermediate_row, source_field_name, source, text)

    def get_weighted_counts(key_to_candidates, key_to_weight, n_items_to_select, weight_name):
        weights_total = sum(
            key_to_weight[key]
            for key in key_to_candidates
        )
        if abs(weights_total - 1) > 1e-6:
            raise ValueError(
                f"build_finetune_rows: {weight_name} must add up to 1; "
                f"got {weights_total}"
            )
        ...
        weighted_keys = [
            key
            for key in key_to_candidates
            if key_to_weight[key] > 0
        ]
        n_available_items = len({
            candidate
            for key in weighted_keys
            for candidate in key_to_candidates[key]
        })
        n_items_to_select = min(n_items_to_select, n_available_items)
        ideal_counts = {
            key: key_to_weight[key] * n_items_to_select
            for key in key_to_candidates
        }
        key_to_count = {
            key: min(
                math.floor(ideal_counts[key]),
                len(key_to_candidates[key]),
            )
            for key in key_to_candidates
        }

        n_remaining_items = n_items_to_select - sum(key_to_count.values())
        while n_remaining_items > 0:
            eligible_keys = [
                key
                for key in weighted_keys
                if key_to_count[key] < len(key_to_candidates[key])
            ]
            ...
            next_key = max(
                eligible_keys,
                key=lambda key: (
                    ideal_counts[key] - key_to_count[key],
                    key_to_weight[key],
                ),
            )
            key_to_count[next_key] += 1
            n_remaining_items -= 1

        return key_to_count

    def get_source_to_negative_candidates(negatives_by_source):
        oracle_negatives = set()
        retrieval_mined_negatives = set()
        for source, negative_texts in negatives_by_source.items():
        ...
        return {
            "oracle": oracle_negatives,
            "retrieval": retrieval_mined_negatives,
        }

    def sample_negatives_from_sources(negatives_by_source):
        source_to_negative_candidates = get_source_to_negative_candidates(negatives_by_source)
        if negative_sampling_mode == "per_query":
            source_to_count = get_weighted_counts(
                source_to_negative_candidates,
                negative_source_to_weight,
                n_negatives_per_sample,
                "negative source weights",
            )
            selected_negatives = []
            for source, n_source_negatives in source_to_count.items():
                selected_negatives.extend(rng.sample(
                    sorted(source_to_negative_candidates[source]),
                    n_source_negatives,
                ))
            return sorted(set(selected_negatives))

        oracle_negatives = source_to_negative_candidates["oracle"]
        retrieval_mined_negatives = source_to_negative_candidates["retrieval"]
        selected_oracle_negatives = rng.sample(
            sorted(oracle_negatives),
            min(n_insufficient_negatives_per_sample, len(oracle_negatives)),
        )
        ...
        return sorted(set(selected_oracle_negatives).union(selected_retrieval_mined_negatives))

    all_query_types = list(QUERY_TYPE_TO_N_MAX_QUERIES) + ["reranker"]
    rng = random.Random(random_seed)
    email_sample_key_to_intermediate_row = {}
    if negative_sampling_mode not in {"per_sample", "per_query"}:
        raise ValueError(
            "build_finetune_rows: negative sampling mode must be one of "
            f"'per_sample' or 'per_query'; got {negative_sampling_mode!r}"
        )
    ...
    add_chunks_to_intermediate_rows("oracle", "supporting_chunks", "positives", "positives_by_source")

    # add oracle insufficient chunks as negatives after all positives are known
    add_chunks_to_intermediate_rows("oracle", "insufficient_chunks", "negatives", "negatives_by_source")

    # add M3 retrieval-mined negatives, excluding oracle positives
    add_chunks_to_intermediate_rows("m3", "retrieval_results", "negatives", "negatives_by_source")
    ...
    for intermediate_row in email_sample_key_to_intermediate_row.values():
        ...
        negative_candidates_by_source = {
            source: sorted(negative_texts)
            for source, negative_texts in intermediate_row["negatives_by_source"].items()
        }
        selected_negatives = sample_negatives_from_sources(negative_candidates_by_source)
        if not selected_negatives:
            continue
        selected_negatives_set = set(selected_negatives)
        ...
        intermediate_rows.append({
            ...
            "negatives_by_source": {
                source: sorted(passage_texts.intersection(selected_negatives_set))
                for source, passage_texts in intermediate_row["negatives_by_source"].items()
                if passage_texts.intersection(selected_negatives_set)
            },
            "negative_candidates_by_source": negative_candidates_by_source,
        })
    ...
    for intermediate_row in intermediate_rows:
        ...
        weighted_query_types = [
            query_type
            for query_type in query_types
            if query_type_to_weight[query_type] > 0
        ]
        query_type_to_count = get_weighted_counts(
            query_type_to_candidates,
            query_type_to_weight,
            n_queries_per_sample,
            "query type weights",
        ...
        n_queries_to_select = min(
            ...
            len({
                query_text
                for query_type in weighted_query_types
                for query_text in query_type_to_candidates[query_type]
            }),
        )
        ...
        for query_text in selected_queries:
            if negative_sampling_mode == "per_query":
                negatives = sample_negatives_from_sources(intermediate_row["negative_candidates_by_source"])
            else:
