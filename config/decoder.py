...
from config.modal_functions import RUN_LOCAL_LM_OR_VLM_LATEST_FUNCTION_NAME
from config.general import USE_FLASH_ATTENTION_IMAGE

SCALEDOWN_WINDOW = 180 # seconds
TIMEOUT = 900 # seconds
...
QUERY_REWRITER_CACHE_TAG = (
    f"kw{QUERY_TYPE_TO_N_MAX_QUERIES['keyword']}"
    f"_nat{QUERY_TYPE_TO_N_MAX_QUERIES['natural']}"
    f"_hyde{QUERY_TYPE_TO_N_MAX_QUERIES['hyde']}"
    f"_q{QUERY_TYPE_TO_N_MAX_QUERIES['question']}"
...
MODEL_PROFILES = {
    EMAIL_WRITER_PROFILE: {
        **EMAIL_WRITER_SETTINGS,
        ...
        "prompt_template": (
            ...
            "3. The incoming email may be current, but the retrieved knowledge-base content may come from earlier years and can contain historical or outdated time-sensitive details. Treat dates, years, calendars, seminar logistics, and other operational details from the retrieved context as potentially stale unless the context clearly supports that they are still current. Use the retrieved context for governing rules, responsible offices, and stable procedures, but avoid presenting time-sensitive retrieved details as certainly current unless they are explicitly confirmed.\n"
            "4. Do not include subject lines or greetings/signatures outside the tags.\n\n"
            "### SAMPLE RESPONSE:\n"
    ...
    THREAD_GROUPER_PROFILE: {
        **THREAD_GROUPER_SETTINGS,
        ...
        "dataset_task_description_start": (
            ...
            "Each email includes: 'threadID', 'author', 'recipients', 'subject', 'body', "
        ...
        "dataset_example": (
            "Input emails:\n"
            f"{{'threadID': 1, 'author': '{EXAMPLE_STUDENT_NAME} <{EXAMPLE_STUDENT_EMAIL}> undefined', 'recipients': '\"[MUIA] {DIRECTOR_NAME}\" <{DIRECTOR_EMAIL}> undefined', 'subject': 'Erasmus', 'body': \"Good morning. I am a student from Italy currently studying at my university. I would like to join the Erasmus program at Universidad Politécnica de Madrid. Would it be possible to take courses from both the MSc in Artificial Intelligence and the Máster Universitario en Automática y Robótica, or should I choose just one? Thank you\"}}\n"
            f"{{'threadID': 1, 'author': '\"[MUIA] {DIRECTOR_NAME}\" <{DIRECTOR_EMAIL}> undefined', 'recipients': '{EXAMPLE_STUDENT_NAME} <{EXAMPLE_STUDENT_EMAIL}> undefined', 'subject': 'Re: Erasmus', 'body': \"I don't know about the other Degree; it is probably taught in another School, so that it can be tricky for you to attend both. Besides, I'm not sure you can follow courses from different School at the same time during your Erasmus. Please ask orex@fi.upm.es: they are in charge of the Erasmus Programme at Escuela Técnica Superior de Ingenieros Informáticos. regards -- {DIRECTOR_NAME} Coordinador del Máster Universitario en Inteligencia Artificial Universidad Politécnica de Madrid Escuela Técnica Superior de Ingenieros Informáticos Campus de Montegancedo S/N, 28660, Boadilla del Monte, Madrid SPAIN Departamento de Inteligencia Artificial {DEPARTMENT_PHONE} {DIRECTOR_EMAIL}\\n\\nOn Mon, 4 May 2020, {EXAMPLE_STUDENT_NAME} <{EXAMPLE_STUDENT_EMAIL}> wrote:\\n> Good morning. I am a student from Italy currently studying at my university. I would like to join the Erasmus program at Universidad Politécnica de Madrid. Would it be possible to take courses from both the MSc in Artificial Intelligence and the Máster Universitario en Automática y Robótica, or should I choose just one? Thank you\"}}\n"
            f"{{'threadID': 2, 'author': '\"[MUIA] {DIRECTOR_NAME}\" <{DIRECTOR_EMAIL}> undefined', 'recipients': '{EXAMPLE_STAFF_EMAIL}', 'subject': 'Estudio de egresados', 'body': \"Hola {EXAMPLE_STAFF_NAME}. Estoy con el informe de titulación. Cuál es el último informe de egresados que tenemos? Hay unos cuantos datos que estaría bien actualizar, como los ex-alumnos que están en el extranjero, los que están realizando un doctorado, etc. Además, dice aquí: El último estudio sobre empleabilidad realizado por el Observatorio Académico sobre titulaciones de postgrado de la ETSIINF es una encuesta fue realizada en el curso 2019/20. Tenemos uno más reciente? Gracias! -- {DIRECTOR_NAME} Coordinador del Máster Universitario en Inteligencia Artificial Universidad Politécnica de Madrid Escuela Técnica Superior de Ingenieros Informáticos Campus de Montegancedo S/N, 28660, Boadilla del Monte, Madrid SPAIN Departamento de Inteligencia Artificial {DEPARTMENT_PHONE} {DIRECTOR_EMAIL}\"}}\n"
            f"{{'threadID': 3, 'author': '{EXAMPLE_STAFF_NAME} <{EXAMPLE_STAFF_EMAIL}>', 'recipients': '\"[MUIA] {DIRECTOR_NAME}\" <{DIRECTOR_EMAIL}> undefined', 'subject': 'Fwd: Fichero egresados', 'body': \"Hola, {DIRECTOR_NAME}. Te reenvío aquí lo último que hice yo. Es de mayo de 2020. No sé si {EXAMPLE_COLLEAGUE_NAME} hizo alguno posterior. ¿Te sirve? {EXAMPLE_STAFF_NAME}\"}}\n"
            f"{{'threadID': 3, 'author': '\"[MUIA] {DIRECTOR_NAME}\" <{DIRECTOR_EMAIL}> undefined', 'recipients': '{EXAMPLE_STAFF_EMAIL}', 'subject': 'Fwd: Fichero egresados', 'body': \"Gracias {EXAMPLE_STAFF_NAME}. A ver lo que puedo sacar de aquí. Un saludo\\n\\nOn Wed, 11 Jun 2020, {EXAMPLE_STAFF_NAME} wrote:\\n> Hola, {DIRECTOR_NAME}. Te reenvío aquí lo último que hice yo. Es de mayo de 2020. No sé si {EXAMPLE_COLLEAGUE_NAME} hizo alguno posterior. ¿Te sirve? {EXAMPLE_STAFF_NAME}\"}}\n"
            f"{{'threadID': 4, 'author': '{EXAMPLE_STUDENT_NAME} <{EXAMPLE_STUDENT_EMAIL}>', 'recipients': '\"[MUIA] {DIRECTOR_NAME}\" <{DIRECTOR_EMAIL}>', 'subject': 'Questions about the programme', 'body': \"A\\n\\n\\nIs the programme taught in Spanish?\\n\\nPlease answer YES or NO\\n\\n\\nB\\n\\n\\nHow long does the master's degree last?\\n\\nPlease indicate the duration\\n\\n\\nC\\n\\n\\nIs the format online?\\n\\nPlease answer YES or NO\\n\\n\\nD\\n\\n\\nDo students work on real case studies?\\n\\nPlease answer YES or NO\"}}\n"
            "Output:\n"
            f"{THREAD_OPENING_TAG}\n"
            f"{THREAD_MESSAGE_OPENING_TAG}\n"
            f"{THREAD_FROM_OPENING_TAG}{EXAMPLE_STUDENT_NAME} <{EXAMPLE_STUDENT_EMAIL}> undefined{THREAD_FROM_CLOSING_TAG}\n"
    ...
    EMAIL_KNOWLEDGE_BASE_CURATOR_PROFILE: {
        **EMAIL_KNOWLEDGE_BASE_CURATOR_SETTINGS,
        ...
        "prompt_template": (
            f"Your job is to process one or more email threads. For each input {THREAD_OPENING_TAG}...{THREAD_CLOSING_TAG} block, output exactly one {THREAD_OPENING_TAG}...{THREAD_CLOSING_TAG} block in the same order. For each thread, either convert it into reusable knowledge for retrieval, or, if the thread does not contain reusable institutional knowledge, output only {NO_USEFUL_INFORMATION_OPENING_TAG}{NO_USEFUL_INFORMATION_CLOSING_TAG} inside that thread.\n"
            "This data will be used by a retrieval system for current and prospective students interested in MUIA (Master's Degree in Artificial Intelligence), coordinated by the Department of Artificial Intelligence (DIA) at FI-UPM.\n"
            "UPM offers many degrees, master's programs, and PhD programs. FI-UPM also offers several different informatics-related degrees, master's programs, and PhD programs. The Department of Artificial Intelligence also appears in contexts that are not only about MUIA.\n"
            "Because of that, scope and contextualization are critical, especially in Q&A pairs.\n"
            "Do not confuse MUIA with other UPM programs.\n"
            ...
            f"1. The input may contain one or more {THREAD_OPENING_TAG}...{THREAD_CLOSING_TAG} blocks.\n"
            f"2. The output must contain exactly one {THREAD_OPENING_TAG}...{THREAD_CLOSING_TAG} block for each input thread.\n"
            "3. Preserve the input order exactly.\n"
            "4. Process each thread independently. Do not merge, split, or move information across threads.\n"
            f"5. Inside each output {THREAD_OPENING_TAG} block, either output only {NO_USEFUL_INFORMATION_OPENING_TAG}{NO_USEFUL_INFORMATION_CLOSING_TAG}, or output the curated fields for that thread.\n\n"
            f"6. All XML-style tags in the output must be well formed. Every opening tag must have exactly one matching closing tag, and a closing tag must never appear before its own opening tag.\n"
            f"7. Inside {QUESTIONS_OPENING_TAG}...{QUESTIONS_CLOSING_TAG}, each pair must be written in this exact order: {QUESTION_OPENING_TAG}...{QUESTION_CLOSING_TAG} immediately followed by {ANSWER_OPENING_TAG}...{ANSWER_CLOSING_TAG}. Do not emit stray closing tags, do not emit {ANSWER_CLOSING_TAG} before {ANSWER_OPENING_TAG}, and do not mix raw 'Q:' or 'A:' text with the XML tags.\n\n"
            "### GENERAL INSTRUCTIONS:\n"
            "1. Write all outputs in English.\n"
            "2. The input is not pre-anonymized. You must anonymize the outputs yourself. Never include the personal name, surname, personal email address, phone number, student identifier, postal address, signature block, CV detail, or any other identifying detail of a current or prospective student, applicant, or other external participant in any output variant. Replace such references with generic roles such as 'the student', 'the applicant', or 'the sender' when needed. Institutional roles, offices, and official university contact points may be kept when they are genuinely useful, but external personal identities must never appear. This also applies when an institutional sender shares external contact details inside the thread: unless the person is clearly presented as an official institutional contact point or representative role, remove or generalize that information instead of preserving the real name or email address.\n"
            "3. Treat institutional answers and institutional outbound messages as the main source of reusable knowledge. This includes replies or standalone messages from the coordinator, the administrative office, official university accounts, or another institutional representative speaking in that role. In practice, likely institutional senders usually use official university, school, department, or program addresses such as @upm.es, @fi.upm.es, or other clearly institutional UPM addresses. Student-side addresses such as personal email domains or student/alumni-style addresses should not by themselves be treated as institutional. Use the sender role and email pattern as evidence, but prioritize the role and message content when they clearly indicate an institutional reply.\n"
            "4. Treat current or prospective students, applicants, and other non-institutional participants as external participants. Information provided only by external participants is not useful by itself unless it is needed to understand institutional information in the same thread.\n"
            ...
            "6. Remove email-only noise from all outputs: greetings, signatures, confidentiality notices, repeated acknowledgements, repeated disclaimers, and quoted or forwarded text that only duplicates information already present elsewhere in the thread. This includes personal sign-offs, footer contact blocks, names in signatures, and other identity-bearing fragments that do not add reusable institutional knowledge.\n"
            ...
            f"3. **Summary**: For each useful thread, produce a detailed, reorganized summary inside {SUMMARY_OPENING_TAG} tags. It should usually be shorter than the cleaned text, but it must retain important specific details such as named regulations, deadlines, conditions, required documents, decision criteria, and alternatives when they appear in the thread.\n"
            f"4. **Q&A Pairs**: For each useful thread, generate up to {Q_AND_A_MAX_PAIRS} Q&A pairs inside {QUESTIONS_OPENING_TAG} tags, using {QUESTION_OPENING_TAG} and {ANSWER_OPENING_TAG} for each pair. Each pair must be structurally complete: one {QUESTION_OPENING_TAG}...{QUESTION_CLOSING_TAG} block and one {ANSWER_OPENING_TAG}...{ANSWER_CLOSING_TAG} block, in that order, with both tags properly closed before the next pair starts. Questions should sound like what another, different student or staff member could naturally ask later, not like curator notes or a recap of one person's case file. Keep each question and answer self-contained, anonymized, and free of vague references such as 'this student', 'this case', or 'this email'. Do not write source-oriented questions that refer to the original text, such as 'which regulation is cited', 'what does the email say', or 'what is mentioned in the message'. Write natural standalone questions about the underlying rule, action, requirement, deadline, meaning, or consequence. Answers must contain the concrete details needed to stand alone. Never include external personal names or identity-bearing details in questions or answers. Only generate a Q&A pair when the thread actually provides a reusable answer. If the thread only contains a question, uncertainty, or a request for information without a substantive institutional answer, do not generate that Q&A pair. Never answer with formulations like 'the applicant asked whether...' or 'the student inquired about...'; those are not answers. If the thread describes a specific student's discrepancy, file history, grade combination, credit count, or one-off decision, extract only the general institutional principle that could help other students in similar situations. If no such general principle is clearly supported, do not generate that Q&A pair.\n"
            ...
            "To: tramitacion.master.oficial@upm.es\n"
            "Subject: Conditional access and pending credits\n\n"
            "Dear team,\n\n"
            "My name is Ana Pérez and I am writing from ana.perez@example.com. I am finishing my bachelor's degree and still have the final project and some credits pending. If my pre-registration is accepted, can I still enroll in the MUIA, and what documents would I need to send afterward?\n"
            "-----\n"
            "-----\n"
            "From: tramitacion.master.oficial@upm.es\n"
            "To: applicant@example.com\n"
            "Cc: masteria.dia@fi.upm.es\n"
            "Subject: Re: Conditional access and pending credits\n\n"
            "Dear student,\n\n"
            ...
            "Subject: Pre-registration documents\n\n"
            "Hello,\n\n"
            "I am interested in pre-registering for the Master in Artificial Intelligence. I have completed almost all the necessary information, but I have questions regarding the documents to be submitted.\n\n"
            "Is it mandatory to submit a motivation letter and/or a letter of recommendation? I cannot find this information on the website.\n\n"
            "If they are not mandatory, would submitting these documents have a positive impact on my candidacy for the master's?\n"
            "-----\n"
            f"{THREAD_CLOSING_TAG}\n\n"
            "**Output**:\n"
            f"{THREAD_OPENING_TAG}\n"
            f"{ABSTRACT_OPENING_TAG}\n"
            "Meaning of conditional access to the MUIA and the requirements for submitting the academic transcript before final admission.\n"
            f"{ABSTRACT_CLOSING_TAG}\n"
            f"{SUMMARY_OPENING_TAG}\n"
            ...
            f"{SUMMARY_CLOSING_TAG}\n"
            f"{CLEANED_TEXT_OPENING_TAG}\n"
            "My name is [REDACTED NAME] and I am writing from [REDACTED EMAIL]. I am finishing the bachelor's degree and still have the bachelor's final project and some credits pending. If the pre-registration is accepted, can I still enroll in the MUIA, and what documents must be sent afterward?\n\n"
            "Once the master's pre-registration documents have been reviewed, conditional access has been granted pending completion of studies. This does not imply final admission, because final admission still depends on the master's program coordinators.\n\n"
            "According to Royal Decree 822/2021, it is possible to enroll in a master's program with only the bachelor's final project and up to 9 ECTS still pending. Therefore, before 15 October 2022, an official academic transcript showing both the completed credits and the credits still pending must be sent through the same communication channel in PDF format and must be smaller than two megabytes.\n"
            f"{CLEANED_TEXT_CLOSING_TAG}\n"
            f"{QUESTIONS_OPENING_TAG}\n"
            ...
            f"{QUESTION_OPENING_TAG}How many pending ECTS are allowed for conditional access to the MUIA in this case?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}Conditional access in this case allows the bachelor's final project and up to 9 ECTS still pending, under Royal Decree 822/2021.{ANSWER_CLOSING_TAG}\n"
            f"{QUESTION_OPENING_TAG}Which regulation allows a student to enroll in the MUIA with the bachelor's final project and some pending credits?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}The regulation that allows this is Royal Decree 822/2021.{ANSWER_CLOSING_TAG}\n"
            ...
            f"{QUESTION_OPENING_TAG}What information must the academic transcript include after conditional access to the MUIA?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}The academic transcript must show both the completed credits and the credits still pending.{ANSWER_CLOSING_TAG}\n"
            f"{QUESTION_OPENING_TAG}When is the deadline for sending the academic transcript after conditional access to the MUIA?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}The academic transcript must be sent before 15 October 2022.{ANSWER_CLOSING_TAG}\n"
            f"{QUESTION_OPENING_TAG}After conditional access to the MUIA, what are the requirements for submitting the official academic transcript?{QUESTION_CLOSING_TAG}\n"
            ...
            f"{QUESTION_OPENING_TAG}Does conditional access to the MUIA depend only on document review, or is there another admission step afterward?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}Conditional access follows review of the pre-registration documents, but final admission still depends on the master's program coordinators.{ANSWER_CLOSING_TAG}\n"
            f"{QUESTIONS_CLOSING_TAG}\n"
            f"{THREAD_CLOSING_TAG}\n"
            f"{THREAD_OPENING_TAG}\n"
            f"{ABSTRACT_OPENING_TAG}\n"
            ...
            f"{CLEANED_TEXT_CLOSING_TAG}\n"
            f"{QUESTIONS_OPENING_TAG}\n"
            f"{QUESTION_OPENING_TAG}Is admission to the MUIA highly competitive?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}Yes. Admission to the MUIA is highly competitive because the number of applicants is much higher than the number of available places.{ANSWER_CLOSING_TAG}\n"
            f"{QUESTION_OPENING_TAG}What kind of academic profile is prioritized for admission to the MUIA?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}Admission decisions prioritize applicants with strong computer science backgrounds and excellent previous academic results. The university of origin may also be considered among other factors.{ANSWER_CLOSING_TAG}\n"
            ...
            f"{QUESTION_OPENING_TAG}Does the applicant's average grade matter when the MUIA evaluates applications?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}Yes. A sufficiently high average grade is one of the factors that matters when the MUIA evaluates applications, and not having a high enough average grade is listed as a common reason for denial.{ANSWER_CLOSING_TAG}\n"
            f"{QUESTION_OPENING_TAG}Does the applicant's university of origin matter when the MUIA evaluates applications?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}Yes. The university of origin may be considered among the factors used when evaluating MUIA applications.{ANSWER_CLOSING_TAG}\n"
            ...
            f"{QUESTION_OPENING_TAG}Can an applicant be rejected from the MUIA for not proving Spanish proficiency?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}Yes. One of the common reasons for denial of admission to the MUIA is not proving Spanish proficiency.{ANSWER_CLOSING_TAG}\n"
            f"{QUESTION_OPENING_TAG}If the MUIA denies admission, does that always mean the applicant has a weak profile?{QUESTION_CLOSING_TAG}\n"
            f"{ANSWER_OPENING_TAG}Not necessarily. Admission can also be denied because the program is highly demanded and the number of applicants is much higher than the number of available places.{ANSWER_CLOSING_TAG}\n"
    ...
    QUERY_REWRITER_PROFILE: {
        **QUERY_REWRITER_SETTINGS,
        ...
        "prompt_template": (
            ...
            "- Use the thread context to resolve what the latest email refers to when the latest message is vague, for example 'any update on this?'.\n"
            "- However, request detection is determined by the latest email only. If the latest email only acknowledges, thanks, confirms understanding, or closes the exchange, output no-request even when earlier context contains an older request.\n"
            "- If the latest email introduces a new question that differs from the earlier thread topic, generate queries for that new latest question and treat the older thread only as background.\n"
            ...
            f"- Output retrieval queries in all four categories: keyword, natural, hyde, and question. Try to output exactly this many queries per category: keyword = {QUERY_TYPE_TO_N_MAX_QUERIES['keyword']}, natural = {QUERY_TYPE_TO_N_MAX_QUERIES['natural']}, hyde = {QUERY_TYPE_TO_N_MAX_QUERIES['hyde']}, question = {QUERY_TYPE_TO_N_MAX_QUERIES['question']}. If it is genuinely difficult to reach the target for a category without producing low-quality duplicates, you may output fewer for that category. Do not exceed these counts, and do not skip a category just because you think another one is better.\n"
            ...
            "To: masteria.dia@fi.upm.es\n"
            "Date: 2026-02-10 09:00:00+00:00\n"
            ...
            "Date: 2026-02-10 12:30:00+00:00\n"
            "Subject: Re: Possible transfer and collaboration with the University Paul Sabatier of Toulouse\n"
            "Body:\n"
            "I need to confirm this with the international office and with the team handling transfer procedures. I will get back to you.\n"
            "---\n"
            "Latest email subject:\n"
            "---\n"
            "Re: Possible transfer and collaboration with the University Paul Sabatier of Toulouse\n"
            "---\n"
            "Latest email body:\n"
            "---\n"
            "Buenos días,\n"
            "¿Tenéis alguna actualización sobre esto y sabéis ya qué oficina lo tramita?\n"
            "---\n"
            "Output:\n"
            f"{QUERIES_OPENING_TAG}\n"
            f"{KEYWORD_QUERIES_OPENING_TAG}\n"
            ...
            f"{QUERY_OPENING_TAG}office for transfer and mobility requests{QUERY_CLOSING_TAG}\n"
            f"{KEYWORD_QUERIES_CLOSING_TAG}\n"
            f"{NATURAL_QUERIES_OPENING_TAG}\n"
            ...
            "Latest email body:\n"
            "---\n"
            "Muchas gracias. Entendido. Quedo a la espera.\n"
            "---\n"
            "Output:\n"
            f"{NO_REQUEST_OPENING_TAG}{NO_REQUEST_CLOSING_TAG}\n\n"
            "### NEW-REQUEST EXAMPLE WITH DIFFERENT LATEST QUESTION:\n"
            "Thread context (oldest to newest, excluding the latest email):\n"
            "---\n"
            "From: student@example.com\n"
            "To: masteria.dia@fi.upm.es\n"
            ...
            "Latest email body:\n"
            "---\n"
            "Además de lo anterior, si finalmente rechazo la plaza después de haber pagado la reserva, ¿qué oficina lo tramita y cuál es el procedimiento?\n"
            "---\n"
            "Output:\n"
            f"{QUERIES_OPENING_TAG}\n"
            f"{KEYWORD_QUERIES_OPENING_TAG}\n"
            f"{QUERY_OPENING_TAG}reservation fee withdrawal procedure{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}decline admitted place after paying reservation fee{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}office responsible for reservation fee cancellation{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}reservation fee refund after declining place{QUERY_CLOSING_TAG}\n"
            f"{KEYWORD_QUERIES_CLOSING_TAG}\n"
            f"{NATURAL_QUERIES_OPENING_TAG}\n"
            f"{QUERY_OPENING_TAG}Procedure to withdraw after paying the reservation fee{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}Office that handles declining an admitted place after the reservation payment{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}Whether the reservation fee can be refunded after giving up the place{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}Administrative steps after deciding not to enroll following spot reservation{QUERY_CLOSING_TAG}\n"
            f"{NATURAL_QUERIES_CLOSING_TAG}\n"
            f"{HYDE_QUERIES_OPENING_TAG}\n"
            f"{QUERY_OPENING_TAG}Students who decline the place after paying the reservation fee must follow the formal withdrawal procedure{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}The reservation payment is handled through the standard admission and enrollment administration{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}Any refund or cancellation after reserving the place depends on the official withdrawal rules and deadlines{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}The responsible office can indicate the administrative steps required after giving up the admitted place{QUERY_CLOSING_TAG}\n"
            f"{HYDE_QUERIES_CLOSING_TAG}\n"
            f"{QUESTION_QUERIES_OPENING_TAG}\n"
            f"{QUERY_OPENING_TAG}What is the procedure to decline an admitted place after paying the reservation fee?{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}Which office handles cancellation after the reservation payment?{QUERY_CLOSING_TAG}\n"
            f"{QUERY_OPENING_TAG}Can the reservation fee be refunded if I finally do not enroll?{QUERY_CLOSING_TAG}\n"
